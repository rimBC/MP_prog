...
-- =============================================================================
-- NOTE: This script does NOT seed the `utilisateur` table.
-- BCrypt password hashes must be generated by the Spring backend so that the
-- bcrypt strength / encoder settings match exactly what `AuthenticationManager`
-- expects at login time. Hard-coding a hash here will work today but silently
-- break the moment the backend's `BCryptPasswordEncoder` configuration changes
-- (different strength, different algorithm version, etc.).
--
-- Create the default users by calling `POST /api/auth/signup` from Postman
-- (see the README section "Creating the Default Credentials via Postman")
-- or from the Swagger UI at http://localhost:8080/api/swagger-ui.html.
-- That way the password is hashed by the running backend with the right
-- encoder, and the resulting hash is guaranteed to verify on login.
-- =============================================================================
BEGIN;
...
('SIMPLE_UTILISATEUR', 'Utilisateur standard'),
...
-- =============================================================================
-- 7. UTILISATEURS — VOLONTAIREMENT NON SEEDÉS
-- =============================================================================
-- Les comptes utilisateurs ne sont PAS insérés ici. Le hash bcrypt doit être
-- généré par le backend Spring (BCryptPasswordEncoder) pour rester cohérent
-- avec le mécanisme de vérification au login. Un hash codé en dur dans ce
-- script casserait silencieusement dès que la configuration de l'encoder
-- changerait (force, version, etc.).
--
-- Créez les comptes par défaut via Postman ou Swagger :
--   POST http://localhost:8080/api/auth/signup
--   (voir la section "Creating the Default Credentials via Postman" du README,
--    ou Swagger UI : http://localhost:8080/api/swagger-ui.html)
--
-- Comptes recommandés (à créer manuellement) :
--   admin    / admin123    -> roleId = 2 (ADMINISTRATEUR)
--   user1    / password123 -> roleId = 1 (SIMPLE_UTILISATEUR)
--   manager1 / password123 -> roleId = 3 (RESPONSABLE)
-- =============================================================================
